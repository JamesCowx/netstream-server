...
/// </summary>
[NetStreamMigration("2026-01-15T12:00:00", nameof(FixIncorrectOwnerIdRelationships))]
...
public class FixIncorrectOwnerIdRelationships : IAsyncMigrationRoutine
{
    ...
    private async Task RemoveDuplicateItemsAsync(NetStreamDbContext context, CancellationToken cancellationToken)
    {
    ...
    private async Task ClearIncorrectOwnerIdsAsync(NetStreamDbContext context, CancellationToken cancellationToken)
    {
    ...
    private async Task ReassignOrphanedExtrasAsync(NetStreamDbContext context, CancellationToken cancellationToken)
    {
    ...
    private async Task PopulatePrimaryVersionIdAsync(NetStreamDbContext context, CancellationToken cancellationToken)
    {
        ...
        var alternateVersionLinks = await context.LinkedChildren
            .Where(lc => (lc.ChildType == NetStream.Database.Implementations.Entities.LinkedChildType.LocalAlternateVersion
                       || lc.ChildType == NetStream.Database.Implementations.Entities.LinkedChildType.LinkedAlternateVersion))
            .Join(
                context.BaseItems,
                lc => lc.ChildId,
                item => item.Id,
