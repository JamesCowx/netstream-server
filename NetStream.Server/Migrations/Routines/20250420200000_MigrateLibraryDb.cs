...
using BaseItemEntity = NetStream.Database.Implementations.Entities.BaseItemEntity;
using Chapter = NetStream.Database.Implementations.Entities.Chapter;
...
/// </summary>
[NetStreamMigration("2025-04-20T20:00:00", nameof(MigrateLibraryDb))]
[NetStreamMigrationBackup(NetStreamDb = true, LegacyLibraryDb = true)]
internal class MigrateLibraryDb : IDatabaseMigrationRoutine
{
    ...
    private readonly IDbContextFactory<NetStreamDbContext> _provider;
    ...
    /// <param name="netstreamDatabaseProvider">The database provider for special access.</param>
    public MigrateLibraryDb(
        IStartupLogger<MigrateLibraryDb> startupLogger,
        IDbContextFactory<NetStreamDbContext> provider,
    ...
    public void Perform()
    {
        _logger.LogInformation("Migrating the userdata from library.db may take a while, do not stop NetStream.");
        ...
        {
            operation.NetStreamDbContext.AttachmentStreamInfos.ExecuteDelete();
            operation.NetStreamDbContext.BaseItems.ExecuteDelete();
            operation.NetStreamDbContext.ItemValues.ExecuteDelete();
            operation.NetStreamDbContext.UserData.ExecuteDelete();
            operation.NetStreamDbContext.MediaStreamInfos.ExecuteDelete();
            operation.NetStreamDbContext.Peoples.ExecuteDelete();
            operation.NetStreamDbContext.PeopleBaseItemMap.ExecuteDelete();
            operation.NetStreamDbContext.Chapters.ExecuteDelete();
            operation.NetStreamDbContext.AncestorIds.ExecuteDelete();
        }
        ...
        using (var operation = GetPreparedDbContext("Moving TypedBaseItem"))
        {
            ...
            using (new TrackedMigrationStep("Clean TypedBaseItems hierarchy", _logger))
            {
                ...
                foreach (var item in allItemsLookup)
                {
                    ...
                    if (DoesResolve(cachedItem.BaseItem.ParentId, checkStack))
                    {
                        ...
                        operation.NetStreamDbContext.BaseItems.Add(cachedItem.BaseItem);
            ...
            using (new TrackedMigrationStep($"Saving {operation.NetStreamDbContext.BaseItems.Local.Count} BaseItem entries", _logger))
            {
                operation.NetStreamDbContext.SaveChanges();
            }
        ...
        using (var operation = GetPreparedDbContext("Moving ItemValues"))
        {
            ...
            using (new TrackedMigrationStep("Loading ItemValues", _logger))
            {
                ...
                {
                    operation.NetStreamDbContext.ItemValues.Add(item.Value.ItemValue);
                    operation.NetStreamDbContext.ItemValuesMap.AddRange(item.Value.ItemIds.Distinct().Select(f => new ItemValueMap()
                    {
                        Item = null!,
                        ItemValue = null!,
                        ItemId = f,
            ...
            using (new TrackedMigrationStep($"Saving {operation.NetStreamDbContext.ItemValues.Local.Count} ItemValues entries", _logger))
            {
                operation.NetStreamDbContext.SaveChanges();
            }
        }
        ...
        using (var operation = GetPreparedDbContext("Moving UserData"))
        {
            ...
            {
                var users = operation.NetStreamDbContext.Users.AsNoTracking().ToArray();
                ...
                foreach (var entity in queryResult)
                {
                    ...
                    operation.NetStreamDbContext.UserData.Add(userData);
                }
            }

            legacyBaseItemWithUserKeys.Clear();

            using (new TrackedMigrationStep($"Saving {operation.NetStreamDbContext.UserData.Local.Count} UserData entries", _logger))
            {
                operation.NetStreamDbContext.SaveChanges();
            }
        }
        ...
        using (var operation = GetPreparedDbContext("Moving MediaStreamInfos"))
        {
            ...
            using (new TrackedMigrationStep("Loading MediaStreamInfos", _logger))
            {
                foreach (SqliteDataReader dto in connection.Query(mediaStreamQuery))
                {
                    ...
                    operation.NetStreamDbContext.MediaStreamInfos.Add(entity);
                }
            }

            using (new TrackedMigrationStep($"Saving {operation.NetStreamDbContext.MediaStreamInfos.Local.Count} MediaStreamInfos entries", _logger))
            {
                operation.NetStreamDbContext.SaveChanges();
            }
        }
        ...
        using (var operation = GetPreparedDbContext("Moving AttachmentStreamInfos"))
        {
            ...
            using (new TrackedMigrationStep("Loading AttachmentStreamInfos", _logger))
            {
                foreach (SqliteDataReader dto in connection.Query(mediaAttachmentQuery))
                {
                    ...
                    operation.NetStreamDbContext.AttachmentStreamInfos.Add(entity);
                }
            }

            using (new TrackedMigrationStep($"Saving {operation.NetStreamDbContext.AttachmentStreamInfos.Local.Count} AttachmentStreamInfos entries", _logger))
            {
                operation.NetStreamDbContext.SaveChanges();
            }
        }
        ...
        using (var operation = GetPreparedDbContext("Moving People"))
        {
            ...
            using (new TrackedMigrationStep("Loading People", _logger))
            {
                ...
                {
                    operation.NetStreamDbContext.Peoples.Add(item.Value.Person);
                    operation.NetStreamDbContext.PeopleBaseItemMap.AddRange(item.Value.Items.DistinctBy(e => (e.ItemId, e.PeopleId)));
                }

                peopleCache.Clear();
            }

            using (new TrackedMigrationStep($"Saving {operation.NetStreamDbContext.Peoples.Local.Count} People entries and {operation.NetStreamDbContext.PeopleBaseItemMap.Local.Count} maps", _logger))
            {
                operation.NetStreamDbContext.SaveChanges();
            }
        }
        ...
        using (var operation = GetPreparedDbContext("Moving Chapters"))
        {
            ...
            using (new TrackedMigrationStep("Loading Chapters", _logger))
            {
                foreach (SqliteDataReader dto in connection.Query(chapterQuery))
                {
                    ...
                    operation.NetStreamDbContext.Chapters.Add(chapter);
                }
            }

            using (new TrackedMigrationStep($"Saving {operation.NetStreamDbContext.Chapters.Local.Count} Chapters entries", _logger))
            {
                operation.NetStreamDbContext.SaveChanges();
            }
        }
        ...
        using (var operation = GetPreparedDbContext("Moving AncestorIds"))
        {
            ...
            using (new TrackedMigrationStep("Loading AncestorIds", _logger))
            {
                foreach (SqliteDataReader dto in connection.Query(ancestorIdsQuery))
                {
                    ...
                    operation.NetStreamDbContext.AncestorIds.Add(ancestorId);
                }
            }

            using (new TrackedMigrationStep($"Saving {operation.NetStreamDbContext.AncestorIds.Local.Count} AncestorId entries", _logger))
            {
                operation.NetStreamDbContext.SaveChanges();
            }
        }
    ...
    {
        public DatabaseMigrationStep(NetStreamDbContext netstreamDbContext, string operationName, ILogger logger) : base(operationName, logger)
        {
            NetStreamDbContext = netstreamDbContext;
        }

        public NetStreamDbContext NetStreamDbContext { get; }
