...
public sealed class NetStreamMigrationAttribute : Attribute
{
    /// <summary>
    /// Initializes a new instance of the <see cref="NetStreamMigrationAttribute"/> class.
    /// </summary>
...
#pragma warning disable CS0618 // Type or member is obsolete
    public NetStreamMigrationAttribute(string order, string name) : this(order, name, null)
#pragma warning restore CS0618 // Type or member is obsolete
    {
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="NetStreamMigrationAttribute"/> class for legacy migrations.
    /// </summary>
    /// <param name="order">The ordering this migration should be applied to. Must be a valid DateTime ISO8601 formatted string.</param>
    /// <param name="name">The name of this Migration.</param>
    ...
    public NetStreamMigrationAttribute(string order, string name, string? key)
    {
        Order = DateTime.Parse(order, CultureInfo.InvariantCulture);
        Name = name;
        Stage = NetStreamMigrationStageTypes.AppInitialisation;
        if (key is not null)
        {
            Key = Guid.Parse(key);
        }
    ...
    /// <summary>
    /// Gets or Sets the stage the annoated migration should be executed at. Defaults to <see cref="NetStreamMigrationStageTypes.CoreInitialisation"/>.
    /// </summary>
    public NetStreamMigrationStageTypes Stage { get; set; } = NetStreamMigrationStageTypes.CoreInitialisation;
