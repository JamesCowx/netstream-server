...
namespace NetStream.Server.Implementations.Tests.Item;
...
public sealed class AlternateVersionQueryTranslationTests : IDisposable
{
    ...
    private static (Guid UserId, Guid PrimaryId, Guid VersionId, Guid OtherId) Seed(NetStreamDbContext ctx)
    {
    ...
    private static (Guid UserId, Guid PrimaryId, Guid VersionAId, Guid VersionBId) SeedTiedVersions(NetStreamDbContext ctx)
    {
        var user = new User("test", "auth-provider", "reset-provider");
        ctx.Users.Add(user);
