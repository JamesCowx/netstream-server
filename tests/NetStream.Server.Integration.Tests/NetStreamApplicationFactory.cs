...
{
    /// <summary>
    /// Factory for bootstrapping the NetStream application in memory for functional end to end tests.
    /// </summary>
    public class NetStreamApplicationFactory : WebApplicationFactory<Startup>
    {
        private static readonly string _testPathRoot = Path.Combine(Path.GetTempPath(), "netstream-test-data");
        private readonly ConcurrentBag<IDisposable> _disposableComponents = new ConcurrentBag<IDisposable>();

        /// <summary>
        /// Initializes static members of the <see cref="NetStreamApplicationFactory"/> class.
        /// </summary>
        static NetStreamApplicationFactory()
        {
        ...
        protected override void ConfigureWebHost(IWebHostBuilder builder)
        {
            ...
            Environment.SetEnvironmentVariable("NETSTREAM_FFMPEG__NOVALIDATION", "true");
            ...
            Directory.CreateDirectory(Path.Combine(webHostPathRoot, "netstream-web"));
            var appPaths = new ServerApplicationPaths(
                webHostPathRoot,
                Path.Combine(webHostPathRoot, "logs"),
                Path.Combine(webHostPathRoot, "config"),
                Path.Combine(webHostPathRoot, "cache"),
                Path.Combine(webHostPathRoot, "netstream-web"));
        ...
        protected override IHost CreateHost(IHostBuilder builder)
        {
            ...
            Program.ApplyCoreMigrationsAsync(appHost.ServiceProvider, Migrations.Stages.NetStreamMigrationStageTypes.CoreInitialisation).GetAwaiter().GetResult();
            appHost.InitializeServices(Mock.Of<IConfiguration>()).GetAwaiter().GetResult();
            Program.ApplyCoreMigrationsAsync(appHost.ServiceProvider, Migrations.Stages.NetStreamMigrationStageTypes.AppInitialisation).GetAwaiter().GetResult();
            host.Start();
