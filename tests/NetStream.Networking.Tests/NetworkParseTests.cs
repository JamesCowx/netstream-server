...
namespace NetStream.Networking.Tests
{
    public class NetworkParseTests
    {
        ...
        [InlineData("netstream.org", "eth16", false, "eth16")]
        // User on external network, no binding - so result is the 1st external.
        [InlineData("netstream.org", "", false, "eth11")]
        ...
        [InlineData("192.168.1.1", "192.168.1.0/24", "eth16,eth11", false, "192.168.1.0/24=internal.netstream", "internal.netstream")]
        ...
        [InlineData("netstream.org", "192.168.1.0/24", "eth16", false, "external=http://helloworld.com", "http://helloworld.com")]

        // User on external network, no binding - so result is the 1st external which is overridden.
        [InlineData("netstream.org", "192.168.1.0/24", "", false, "external=http://helloworld.com", "http://helloworld.com")]
        ...
        [InlineData("192.168.1.1", "192.168.1.0/24=internal.netstream:8097", "internal.netstream", 8097)]
        // External/all override with an explicit port.
        [InlineData("8.8.8.8", "all=external.netstream:8097", "external.netstream", 8097)]
        ...
        [InlineData("8.8.8.8", "all=https://secure.netstream.org:8920", "https://secure.netstream.org:8920", null)]
        ...
        [InlineData("all=https://media.example.com", "/netstream", true)]
        // Full-URL override that ends with the base URL (with and without a trailing slash): no warning.
        [InlineData("all=https://media.example.com/netstream", "/netstream", false)]
        [InlineData("all=https://media.example.com/netstream/", "/netstream", false)]
        [InlineData("all=https://media.example.com/media/netstream", "/netstream", false)]
        ...
        [InlineData("all=https://media.example.com/netstreamx", "/netstream", true)]
        [InlineData("all=https://media.example.com/netstream/media", "/netstream", true)]
        ...
        [InlineData("all=media.example.com", "/netstream", false)]
        [InlineData("internal=http-proxy.lan:8097", "/netstream", false)]
        // Keyword overrides go through the same check as "all".
        [InlineData("internal=http://10.0.0.5:8096", "/netstream", true)]
        ...
        /// <summary>
        /// The NETSTREAM_PublishedServerUrl environment variable / --published-server-url option takes the
        ...
        private static void VerifyBaseUrlWarning(Mock<ILogger<NetworkManager>> logger, Times times)
        {
            logger.Verify(
                l => l.Log(
                    LogLevel.Warning,
                    It.IsAny<EventId>(),
                    It.Is<It.IsAnyType>((state, _) => state.ToString()!.Contains("NetStream will append this base URL when generating Live TV client URLs", StringComparison.Ordinal)),
                    It.IsAny<Exception?>(),
                    It.IsAny<Func<It.IsAnyType, Exception?, string>>()),
                times);
        }
