...
namespace NetStream.Api.Tests.Helpers
{
    public class MediaInfoHelperTests
    {
        ...
        public async Task OpenMediaSource_ExternalDockerBridgeBehindReverseProxy_UsesPublishedUrl()
        {
            ...
            var appHost = new Mock<IServerApplicationHost>();
            appHost.Setup(x => x.GetSmartApiUrl(It.IsAny<HttpRequest>())).Returns("https://netstream.example.com");

            var helper = CreateHelper(mediaSourceManager: mediaSourceManager.Object, appHost: appHost.Object);

            var response = await helper.OpenMediaSource(new DefaultHttpContext(), new LiveStreamRequest()).ConfigureAwait(true);

            Assert.Equal("https://netstream.example.com" + LiveStreamFilesPath, response.MediaSource.Path);
        ...
        public async Task OpenMediaSource_BaseUrlConfigured_RewritesWithBaseUrlPrefix()
        {
            var mediaSource = new MediaSourceInfo
            {
                Id = "abc",
                Protocol = MediaProtocol.Http,
                Path = "http://172.19.0.3:8096/netstream" + LiveStreamFilesPath,
                LiveStreamId = "livestream-1"
            };

            var helper = CreateOpenMediaSourceHelper(mediaSource, "https://media.example.com/netstream", "/netstream");

            var response = await helper.OpenMediaSource(new DefaultHttpContext(), new LiveStreamRequest()).ConfigureAwait(true);

            Assert.Equal("https://media.example.com/netstream" + LiveStreamFilesPath, response.MediaSource.Path);
        }

        [Fact]
        public async Task OpenMediaSource_BaseUrlSegmentMismatch_PathUnchanged()
        {
            const string LocalPath = "http://172.19.0.3:8096/netstream2" + LiveStreamFilesPath;
        ...
        public async Task OpenMediaSource_ExplicitPortOverrideWithBaseUrl_RewritesToOverrideHostAndPort()
        {
            ...
            var helper = CreateOpenMediaSourceHelper(mediaSource, "http://myhost:8097/netstream", "/netstream");

            var response = await helper.OpenMediaSource(new DefaultHttpContext(), new LiveStreamRequest()).ConfigureAwait(true);

            Assert.Equal("http://myhost:8097/netstream" + LiveStreamFilesPath, response.MediaSource.Path);
        }
        ...
        [InlineData(
            "https://media.example.com",
            "http://172.19.0.3:8096/netstream" + LiveStreamFilesPath,
            MediaProtocol.Http,
            "/netstream",
            "https://media.example.com/netstream" + LiveStreamFilesPath)]
        [InlineData(
            "https://netstream",
            "http://172.19.0.3:8096/netstream" + LiveStreamFilesPath,
            MediaProtocol.Http,
            "/netstream",
            "https://netstream/netstream" + LiveStreamFilesPath)]
        [InlineData(
            "https://media.example.com/netstream",
            "http://172.19.0.3:8096/netstream" + LiveStreamFilesPath,
            MediaProtocol.Http,
            "/netstream",
            "https://media.example.com/netstream" + LiveStreamFilesPath)]
        ...
        [InlineData(
            "https://media.example.com",
            "http://172.19.0.3:8096/netstream2" + LiveStreamFilesPath,
            MediaProtocol.Http,
            "/netstream",
            null)]
        [InlineData(
