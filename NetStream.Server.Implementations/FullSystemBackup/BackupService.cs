...
namespace NetStream.Server.Implementations.FullSystemBackup;
...
public class BackupService : IBackupService
{
    ...
    /// <param name="netstreamDatabaseProvider">The NetStream database Provider in use.</param>
    ...
    public async Task RestoreBackupAsync(string archivePath)
    {
        ...
        await using (fileStream.ConfigureAwait(false))
        {
            ...
            {
                throw new NotSupportedException($"The loaded archive '{archivePath}' does not appear to be a NetStream backup as its missing the '{ManifestEntryName}'.");
            }
            ...
            if (manifest!.ServerVersion > _applicationHost.ApplicationVersion) // newer versions of NetStream should be able to load older versions as we have migrations.
            {
                throw new NotSupportedException($"The loaded archive '{archivePath}' is made for a newer version of NetStream ({manifest.ServerVersion}) and cannot be loaded in this version.");
            }
            ...
            if (manifest.Options.Database)
            {
                ...
                await using (dbContext.ConfigureAwait(false))
                {
                    ...
                    {
                        _logger.LogInformation("No backup of the history table in archive. This is required for NetStream operation");
                    ...
                    var entityTypes = typeof(NetStreamDbContext).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)
                    ...
                    await _netstreamDatabaseProvider.PurgeDatabase(dbContext, tableNames).ConfigureAwait(false);
            ...
            _logger.LogInformation("Restored NetStream system from {Date}", manifest.DateCreated);
        }
    }
    ...
    public async Task<BackupManifestDto> CreateBackupAsync(BackupOptionsDto backupOptions)
    {
        ...
        await _netstreamDatabaseProvider.RunScheduledOptimisation(CancellationToken.None).ConfigureAwait(false);
        ...
        var backupPath = Path.Combine(backupFolder, $"netstream-backup-{manifest.DateCreated.ToLocalTime():yyyyMMddHHmmss}.zip");
