...
public static class OrderMapper
{
    ...
    /// <param name="netstreamDbContext">Context.</param>
    /// <returns>Func to be executed later for sorting query.</returns>
    public static Expression<Func<BaseItemEntity, object?>> MapOrderByField(ItemSortBy sortBy, InternalItemsQuery query, NetStreamDbContext netstreamDbContext)
    {
        if (sortBy == ItemSortBy.DatePlayed)
        {
            ...
            var userData = query.User is null
                ? netstreamDbContext.UserData
                : netstreamDbContext.UserData.Where(w => w.UserId == query.User.Id);
