...
using NetStream.Database.Implementations.DbConfiguration;
using NetStream.Database.Implementations.Locking;
using NetStream.Database.Providers.Sqlite;
using MediaBrowser.Common.Configuration;
using MediaBrowser.Controller.Configuration;
using Microsoft.EntityFrameworkCore;
using Microsoft.Extensions.Configuration;
using Microsoft.Extensions.DependencyInjection;
using NetStreamDbProviderFactory = System.Func<System.IServiceProvider, NetStream.Database.Implementations.INetStreamDatabaseProvider>;

namespace NetStream.Server.Implementations.Extensions;
...
public static class ServiceCollectionExtensions
{
    ...
    private static IDictionary<string, NetStreamDbProviderFactory> GetSupportedDbProviders()
    {
        var items = new Dictionary<string, NetStreamDbProviderFactory>(StringComparer.InvariantCultureIgnoreCase);
        foreach (var providerType in DatabaseProviderTypes())
        {
            var keyAttribute = providerType.GetCustomAttribute<NetStreamDatabaseProviderKeyAttribute>();
            ...
            items[keyAttribute.DatabaseProviderKey] = (services) => (INetStreamDatabaseProvider)ActivatorUtilities.CreateInstance(services, providerType);
        }

        return items;
    }

    private static NetStreamDbProviderFactory? LoadDatabasePlugin(CustomDatabaseOptions customProviderOptions, IApplicationPaths applicationPaths)
    {
        ...
        var dbProviderType = assembly.GetExportedTypes().FirstOrDefault(f => f.IsAssignableTo(typeof(INetStreamDatabaseProvider)))
            ?? throw new InvalidOperationException($"Could not find any type implementing the '{nameof(INetStreamDatabaseProvider)}' interface.");

        return (services) => (INetStreamDatabaseProvider)ActivatorUtilities.CreateInstance(services, dbProviderType);
    }
    ...
    public static IServiceCollection AddNetStreamDbContext(
        ...
        IConfiguration configuration)
    {
        ...
        NetStreamDbProviderFactory? providerFactory = null;
        ...
        else
        {
            ...
            {
                throw new InvalidOperationException($"NetStream cannot find the database provider of type '{efCoreConfiguration.DatabaseType}'. Supported types are {string.Join(", ", providers.Keys)}");
            }
        }

        serviceCollection.AddSingleton<INetStreamDatabaseProvider>(providerFactory!);
        ...
        serviceCollection.AddPooledDbContextFactory<NetStreamDbContext>((serviceProvider, opt) =>
        {
            var provider = serviceProvider.GetRequiredService<INetStreamDatabaseProvider>();
