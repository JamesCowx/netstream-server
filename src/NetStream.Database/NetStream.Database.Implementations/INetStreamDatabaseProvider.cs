...
/// </summary>
public interface INetStreamDatabaseProvider
{
    /// <summary>
    /// Gets or Sets the Database Factory when initialisaition is done.
    /// </summary>
    IDbContextFactory<NetStreamDbContext>? DbContextFactory { get; set; }

    /// <summary>
    /// Initialises netstreams EFCore database access.
    /// </summary>
    /// <param name="options">The EFCore database options.</param>
    /// <param name="databaseConfiguration">The NetStream database options.</param>
    ...
    /// <summary>
    /// If supported this should perform any actions that are required on stopping the netstream server.
    /// </summary>
    ...
    Task PurgeDatabase(NetStreamDbContext dbContext, IEnumerable<string>? tableNames);
}
