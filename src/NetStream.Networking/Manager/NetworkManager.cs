...
namespace NetStream.Networking.Manager;
...
public class NetworkManager : INetworkManager, IDisposable
{
    ...
    /// URL. NetStream appends the base URL to generated Live TV client URLs in this case, which can conflict with
    ...
    private void WarnIfPublishedUrlBasePathDiffers(List<PublishedServerUriOverride> publishedServerUrls, string baseUrl)
    {
        ...
        foreach (var overrideUri in publishedServerUrls.Select(x => x.OverrideUri).Distinct(StringComparer.OrdinalIgnoreCase))
        {
            ...
            _logger.LogWarning(
                "The published server URL for host '{PublishedServerHost}' does not end with the configured base URL '{BaseUrl}'. NetStream will append this base URL when generating Live TV client URLs. If your reverse proxy translates public paths, this may cause Live TV playback to fail. Update the Published Server URIs setting on the Networking page of the admin dashboard, the NETSTREAM_PublishedServerUrl environment variable / --published-server-url option, or the reverse proxy path mapping accordingly.",
                publishedServerHost,
                baseUrl);
        }
    }
    ...
    /// <summary>
    /// Reads the netstream configuration of the configuration manager and produces a list of interfaces that should be bound.
    /// </summary>
    /// <param name="logger">Logger to use for messages.</param>
    ...
    /// <returns>A list of ip address of which netstream should bind to.</returns>
    ...
    public string GetBindAddress(IPAddress? source, out int? port, bool skipOverrides = false)
    {
        ...
        if (source is not null)
        {
            ...
            {
                _logger.LogWarning("IPv6 is disabled in NetStream, but enabled in the OS. This may affect how the interface is selected.");
            }

            if (!IsIPv4Enabled && IsIPv6Enabled && source.AddressFamily == AddressFamily.InterNetwork)
            {
                _logger.LogWarning("IPv4 is disabled in NetStream, but enabled in the OS. This may affect how the interface is selected.");
            }
